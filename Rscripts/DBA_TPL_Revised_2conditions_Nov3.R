...
#' @description Script that perform differential analysis of samples esposed to TPL(in DMSO) vs DMSO (control).
...
path1='/Users/simeon01/Documents/Karen/20201023_revise_TPL_DRB/TPL_coverages'

setwd(path1)
pattern_file_extension=".consensus_noTPL_10uM_TPL_2hr.merged.bed"
...
norm_Matrix_cov_selected <- norm_sign_to_tot_map_reads(Matrix_cov_selected,stat5_files,".consensus_noTPL_10uM_TPL_2hr.merged.bed")
norm_Matrix_cov <- norm_sign_to_tot_map_reads(Matrix_cov,stat5_files,".consensus_noTPL_10uM_TPL_2hr.merged.bed")
...
num_TPL_2h<- length(grep('^10uM_TPL_2hr',temp_names))
num_no_TPL <- length(grep('^no_TPL',temp_names))

                                          
conditions <- temp_names
conditions[grep('^10uM_TPL_2hr',temp_names)] <- "TPL_2hr"
conditions[grep('^no_TPL',temp_names)] <- "no_TPL"

conditions 
# [1] "TPL_2hr" "TPL_2hr" "TPL_2hr" "TPL_2hr" "TPL_2hr" "TPL_2hr" "noTPL"  
# [8] "noTPL"   "noTPL"   "noTPL"   "noTPL"   "noTPL"   "noTPL"   "noTPL"  
#[15] "noTPL"   "noTPL"   "noTPL"   "noTPL"    

conditions <- factor(conditions) # there are 2 factors
conditions <- relevel(conditions,ref="no_TPL") # relevel as reference base the condition no_TPL_1hr_no_dm6
biorep <- gsub('.i7*.*','',gsub('.*.SLX','SLX',temp_names)) 
biorep
# [1] "SLX-18053" "SLX-18053" "SLX-18053" "SLX-18059" "SLX-18059" "SLX-18059"
# [7] "SLX-16410" "SLX-16410" "SLX-16410" "SLX-15870" "SLX-15870" "SLX-15870"
#[13] "SLX-18053" "SLX-18053" "SLX-18053" "SLX-18059" "SLX-18059" "SLX-18059"     
...
#dir_selected="/Users/simeon01/Documents/Karen/20201023_revise_TPL_DRB/TPL_coverages/only_2Cond_TPL_updated_filter_over_input_thr0_TSS_plus_minus500"
...
colnames(fit)
#[1] "(Intercept)"       "conditionsTPL_2hr" "biorepSLX-16410"  
#[4] "biorepSLX-18053"   "biorepSLX-18059

##  =========== DRB_1hr_vs_noDRB ================
contrast_TPL_vs_noTPL <- c(0,1,0,0,0)
colnames(fit)[which(contrast_TPL_vs_noTPL==1)]
label_case <-  "_TPL_vs_noTPL_fdr0.01"

qlf.TPL_vs_noTPL <- glmQLFTest(fit, contrast = contrast_TPL_vs_noTPL) 

de_pairing_TPL_vs_noTPL <- topTags(qlf.TPL_vs_noTPL, n= nrow(y$counts))$table

write.table(de_pairing_TPL_vs_noTPL,file = paste0('EdgeRtable_differential_binding_analysis_',label_case,'.csv'), col.names = NA,sep = ",",quote = F)


res_TPL_vs_noTPL <- decideTestsDGE(qlf.TPL_vs_noTPL,adjust.method="fdr", p.value = 0.01,lfc=0)
#summary(res_TPL_vs_noTPL)

summary_res_TPL_vs_noTPL <- summary(res_TPL_vs_noTPL)

tss_res_PDS <- export_bed_and_TSS_in_bed_regions(tss_file,'./',label_case,res_TPL_vs_noTPL)

pdf(paste0('MD_MA_plot_differential_binding_analysis_',label_case,'.pdf'),8,7)
plotMD(qlf.TPL_vs_noTPL,status = res_TPL_vs_noTPL,values =c(1,0,-1),col=c("#ff000080","#00000030","#0000ff80"),pch = 19,cex=c(1,0.8,1))
title(paste0("\n\nDEup:",length(which(res_TPL_vs_noTPL==1)),
                           " - DEdown: ",length(which(res_TPL_vs_noTPL==-1)),
                           " - notDE: ",length(which(res_TPL_vs_noTPL==0))," - Tot: ",length(res_TPL_vs_noTPL)))
dev.off()

message("-------------------------- .. -----------------------------")
message('fdr=0.01')
print(cbind(summary(res_TPL_vs_noTPL), summary(res_TPL_vs_noTPL)/sum(summary(res_TPL_vs_noTPL))))
res_TPL_vs_noTPL_fdr0.01 <- res_TPL_vs_noTPL
message("-------------------------- .. -----------------------------")

res_TPL_vs_noTPL <- decideTestsDGE(qlf.TPL_vs_noTPL,adjust.method="fdr", p.value = 0.05,lfc=0)
label_case <-  "_TPL_vs_noTPL_fdr0.05"
summary(res_TPL_vs_noTPL)
...
message('fdr=0.05')
print(cbind(summary(res_TPL_vs_noTPL), summary(res_TPL_vs_noTPL)/sum(summary(res_TPL_vs_noTPL))))
res_TPL_vs_noTPL_fdr0.05 <- res_TPL_vs_noTPL
message("-------------------------- .. -----------------------------")


res_TPL_vs_noTPL <- decideTestsDGE(qlf.TPL_vs_noTPL,adjust.method="none", p.value = 0.01,lfc=0)
label_case <-  "_TPL_vs_noTPL_pval0.01"
#summary(res_TPL_vs_noTPL)
...
message('pval=0.01')
print(cbind(summary(res_TPL_vs_noTPL), summary(res_TPL_vs_noTPL)/sum(summary(res_TPL_vs_noTPL))))
res_TPL_vs_noTPL_pval0.01 <- res_TPL_vs_noTPL
message("-------------------------- .. -----------------------------")

res_TPL_vs_noTPL <- decideTestsDGE(qlf.TPL_vs_noTPL,adjust.method="none", p.value = 0.05,lfc=0)
label_case <-  "_TPL_vs_noTPL_pval0.05"
summary(res_TPL_vs_noTPL)
...
message('pval=0.05')
print(cbind(summary(res_TPL_vs_noTPL), summary(res_TPL_vs_noTPL)/sum(summary(res_TPL_vs_noTPL))))
res_TPL_vs_noTPL_pval0.05 <- res_TPL_vs_noTPL
message("-------------------------- .. -----------------------------")
...
SUMM <- cbind(summary(res_TPL_vs_noTPL_fdr0.01),summary(res_TPL_vs_noTPL_fdr0.05),summary(res_TPL_vs_noTPL_pval0.01),summary(res_TPL_vs_noTPL_pval0.05))
print(SUMM)
colnames(SUMM) <- c('TPL_vs_noTPL_fdr0.01','TPL_vs_noTPL_fdr0.05','TPL_vs_noTPL_pval0.01',
                    'TPL_vs_noTPL_pval0.05')
print(SUMM)
write.table(SUMM,file='summary_all_cases_DBA.TPL.csv',quote = F, sep = ",", col.names = NA)
